...
def easy_wrapped_loss(x):
    x_UK = [-0.1610901653766632, -1.5463283061981201, -0.8385720252990723, -0.6723966002464294, 0.25519952178001404, -0.5561386942863464, x,  0.37870895862579346, 0.0023782984353601933, 0.4264898896217346, 1.7800384759902954, 1.1601382493972778, 1.0630438327789307, 0.5169200897216797, 3.795074462890625]
    x_US = [0.19965657591819763, 6.073495388031006, -0.9819183945655823, 0.647635281085968, 0.43982991576194763, 0.8020678162574768, x, 0.23580731451511383,  0.00011244539200561121, 0.44379621744155884, 1.5602449178695679, 4.342600345611572, 0.02553415112197399, 0.4821716845035553, 1.2816518545150757]
    result = US_logloss(*tuple(x_US)) +UK_logloss(*tuple(x_UK)) # Unpack the parameters from the tuple
    return -result.item()
best_params_arr = []
best_values_arr = []
x_UK = [-0.1610901653766632, -1.5463283061981201, -0.8385720252990723, -0.6723966002464294, 0.25519952178001404, -0.5561386942863464, 1.8897273540496826, 0.37870895862579346, 0.0023782984353601933, 0.4264898896217346, 1.7800384759902954, 1.1601382493972778, 1.0630438327789307, 0.5169200897216797, 3.795074462890625]
x_US = [0.19965657591819763, 6.073495388031006, -0.9819183945655823, 0.647635281085968, 0.43982991576194763, 0.8020678162574768, 1.4228870526267201e-08, 0.23580731451511383, 2.400108112254884e-09, 0.4377947449684143, 5.935051441192627, 4.291709899902344, 0.025035476312041283, 0.4777671694755554, 4.93689489364624] 
keys = jr.split(jr.PRNGKey(42) , 1001)  # Generate 10 random keys
initial_beta = [float(jr.uniform(key, shape=(1,), minval=0, maxval=3)[0]) for key in keys]

for m in range(1001):
    initial_val = initial_beta[m]
    ...
    np.save(f'same_cost_p.npy', best_params_arr)
    np.save(f'same_cost_v.npy', best_values_arr)
