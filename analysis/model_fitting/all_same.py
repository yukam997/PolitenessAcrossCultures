    ...
    listener: thinks[
        ...
        speaker: chooses(w in W, wpp=
            imagine[
                ...
                (Pr[listener.s == s])**(0.3*inf_term)*exp(100*soc_term * U_soc(w,p,country) - # U_soc = listener's EU
...
def easy_wrapped_loss(x):
    result = US_logloss(*tuple(x))+UK_logloss(*tuple(x)) # Unpack the parameters from the tuple
    return -result.item()
best_params_arr = []
best_values_arr = []
...
for m in range(1001):
    initial_val = list(jr.uniform(keys[m], shape=(6,), minval=-3, maxval=3))+ list(jr.uniform(keys[m], shape=(9,), minval=0, maxval= 3))  # Generate\random numbers between 0 and 3
    ...
    try:
        np.save(f'all_same_p.npy', best_params_arr)
        np.save(f'all_same_v.npy', best_values_arr)
    except:
        continue
